...
-(void)adjustContentOffsetWithCenterX:(CGFloat)centerX {
    ...
    if (centerX > self.scrollView.width / 2) {
        if (centerX < self.scrollView.contentSize.width - self.scrollView.width / 2) {
            [self.scrollView setContentOffset:CGPointMake(self.indicatorBar.centerX - self.scrollView.width / 2 - countcoordinatesX(15), 0) animated:YES];
        }else {
            [self.scrollView setContentOffset:CGPointMake(self.scrollView.contentSize.width - self.scrollView.width + countcoordinatesX(15), 0) animated:YES];
        }
    }else {
        [self.scrollView setContentOffset:CGPointMake(0 - countcoordinatesX(15), 0) animated:YES];
    }
}
...
-(void)setSegmentedTitles:(NSArray<LRLChannelUnitModel *> *)segmentedTitles {
    ...
    for (LRLChannelUnitModel *model in self.segmentedTitles) {
        ...
        [segBtn sizeToFit];
        [segBtn setBounds:CGRectMake(0, 0, segBtn.titleLabel.intrinsicContentSize.width, segBtn.titleLabel.intrinsicContentSize.height)];
...
-(UIScrollView *)scrollView {
    if (!_scrollView) {
        ...
        _scrollView.contentInset = UIEdgeInsetsMake(0, countcoordinatesX(15), 0, countcoordinatesX(15));
    }
    return _scrollView;
}
-(UIView *)indicatorBar {
